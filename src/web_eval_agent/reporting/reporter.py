...
from ..core.test_executor import TestResults, TestResult
from ..utils.utils import format_duration, ensure_directory, get_file_size_mb
...
class Reporter:
    ...
    async def _generate_text_report(self, results: TestResults) -> str:
        ...
        elif self.config.report_detail_level == "structured":
            text_content = self._create_structured_text_report(results)
...
================================================================================"""
    ...
    def _create_structured_text_report(self, results: TestResults) -> str:
        """Create a highly structured text report with clear sections and data tables."""
        lines = []
        
        # Header Section
        lines.extend([
            "╔" + "═" * 98 + "╗",
            "║" + " " * 30 + "WEB EVALUATION AGENT - STRUCTURED REPORT" + " " * 27 + "║",
            "╚" + "═" * 98 + "╝",
            "",
            f"Report Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            f"Target URL: {self.config.url}",
            f"Test Duration: {format_duration(results.total_duration)}",
            ""
        ])
        ...
        lines.extend([
            ...
            f"│ Total Tests:     {total_tests:>3} │ Passed:     {passed_tests:>3} │ Failed:     {failed_tests:>3} │ Success Rate: {success_rate:>6.1f}% │",
            f"│ Browser:    {self.config.browser:>8} │ Viewport: {self.config.viewport:>11} │ Mode: {'Headless':>9} │ Duration: {format_duration(results.total_duration):>9} │",
            "└" + "─" * 98 + "┘",
            ""
        ])
        ...
        if results.test_results:
            lines.extend([
                "┌─ TEST EXECUTION RESULTS " + "─" * 72 + "┐",
                "│ # │ Status │ Test Name" + " " * 45 + "│ Duration │ Validations │",
                "├───┼────────┼" + "─" * 54 + "┼──────────┼─────────────┤"
            ])
            
            for i, result in enumerate(results.test_results, 1):
                status = "PASS" if result.passed else "FAIL"
                name = result.scenario_name[:50] + "..." if len(result.scenario_name) > 50 else result.scenario_name
                duration = format_duration(result.duration)
                validations = f"{sum(1 for v in result.validation_results if v.get('passed', False))}/{len(result.validation_results)}" if result.validation_results else "0/0"
                
                lines.append(f"│{i:>2} │ {status:>6} │ {name:<54} │ {duration:>8} │ {validations:>11} │")
        ...
        if results.test_results:
            lines.extend([
                "┌─ DETAILED TEST ANALYSIS " + "─" * 72 + "┐"
            ])
            
            for i, result in enumerate(results.test_results, 1):
                status_symbol = "✓" if result.passed else "✗"
                lines.extend([
                    f"│ Test {i}: {status_symbol} {result.scenario_name}",
                    f"│   Duration: {format_duration(result.duration)}",
                    f"│   Console Logs: {len(result.console_logs)} entries",
                    f"│   Network Requests: {len(result.network_requests)} requests",
                    f"│   Screenshots: {len(result.screenshots)} captured"
                ])
                ...
                if result.validation_results:
                    ...
                    for validation in result.validation_results:
                        val_status = "✓" if validation.get("passed", False) else "✗"
                        val_name = validation.get("name", "Unknown")
                        lines.append(f"│     {val_status} {val_name}")
        ...
        # Performance Metrics
        if results.test_results:
            total_logs = sum(len(r.console_logs) for r in results.test_results)
            total_requests = sum(len(r.network_requests) for r in results.test_results)
            total_screenshots = sum(len(r.screenshots) for r in results.test_results)
            avg_duration = results.total_duration / len(results.test_results) if results.test_results else 0
            
            lines.extend([
                "┌─ PERFORMANCE METRICS " + "─" * 75 + "┐",
                f"│ Average Test Duration: {format_duration(avg_duration):>15} │ Total Console Logs: {total_logs:>8} │",
                f"│ Total Network Requests: {total_requests:>14} │ Total Screenshots: {total_screenshots:>9} │",
                "└" + "─" * 98 + "┘",
                ""
            ])
        ...
        lines.extend([
            ...
            f"│ Browser: {self.config.browser:<20} │ Headless Mode: {str(self.config.headless):<10} │",
            f"│ Viewport: {self.config.viewport:<19} │ Output Format: {self.config.report_format:<10} │",
            f"│ Detail Level: {self.config.report_detail_level:<14} │ Target URL: {self.config.url[:25]:<25} │",
            "└" + "─" * 98 + "┘",
            ""
        ])
        ...
        lines.extend([
            "─" * 100,
            f"Report generated by Web Eval Agent v2.0.0 at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            "─" * 100
        ])
